...
| R2 Context And Safety / 上下文安全 | `context-engine`, `memory-cache-management`, `usage-budget-management`, `workspace-state-management`, `code-intelligence` | projection, redaction, compaction, budget, sandbox matrix, checkpoint/undo. / projection、redaction、compaction、budget、sandbox matrix、checkpoint/undo。 |
