...
| R2 Context And Safety / 上下文安全 | `context-engine`, `memory-cache-management`, `usage-budget-management`, `workspace-state-management`, `code-intelligence` | projection, redaction, compaction, budget, sandbox matrix, checkpoint/undo, code intelligence evidence. / projection、redaction、compaction、budget、sandbox matrix、checkpoint/undo、code intelligence evidence。 |
| R3 Extensibility / 扩展 | `skill-system`, `hook-system`, `mcp-gateway`, `plugin-system`, `extension-system` | manifests, permission diff, lockfile, gateway policy, hook ordering. / manifest、权限 diff、lockfile、gateway policy、hook ordering。 |
| R4 IDE And Server / IDE 与 Server | `communication-protocol`, `remote-runtime-connectivity`, `runtime-message-bus`, `session-store` | protocol compatibility, VSCode projection, server transport, SDK schema. / protocol compatibility、VSCode projection、server transport、SDK schema。 |
