...
| Testing/regression / 测试回归 | Hooks and commands can enforce local workflows. / hooks 与 commands 可强制本地流程。 | Agents SDK has evaluation concepts and Codex workflows. / Agents SDK 有 eval 概念与 Codex workflows。 | Deterministic fakes, golden replay, matrix, versioning, e2e, optional live tests. / 确定性 fake、golden、matrix、versioning、e2e、optional live。 | Regression is platform infrastructure. / 回归是平台基础设施。 |
