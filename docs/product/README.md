...
This volume explains where DeepSeek CLI is going as a product, how it compares to Claude Code and Codex class tools, and how product roadmap nodes map to architecture packages.

本卷解释 DeepSeek CLI 的产品方向、如何对标 Claude Code 和 Codex 类工具，以及产品路线图节点如何映射到架构包。
...
DeepSeek CLI should become a governed AI engineering runtime, not just a terminal assistant.

DeepSeek CLI 应成为受治理的 AI 工程运行时，而不只是终端助手。
...
- task graphs and subagents / task graph 与 subagents
- replay, audit, and regression / replay、audit 和 regression
- future server/cloud/native hosts / 未来 server、cloud、native hosts
