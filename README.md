...
```
Intent → Preflight → Policy → Scheduler → Execute → Evidence → Host
         (归一化)    (安全门)   (调度)      (执行)    (审计证据)  (渲染)
```
...
### Step 4: Orchestration schedules work / 编排调度任务
...
| ⚙️ **Runtime** | `runtime` · `session-store` | Turn lifecycle, session, replay / 生命周期与会话 |
...
| 🤖 **AI/Context** | `model-gateway` · `context-engine` · `memory-cache-management` · `code-intelligence` | Provider isolation, context, memory / AI 与上下文 |
...
| 🖥️ CLI host | 🟡 Headless + basic local flows; rich TUI planned |
...
| 🧩 Extensibility | 🟡 Landing zones for skills, hooks, MCP, plugins, subagents |
...
| **R6** | Rich TUI, keybindings, notifications, voice/native, browser bridge |
