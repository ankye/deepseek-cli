import type {
  AgentContinueRequest,
  AgentSpawner,
  CapabilityExecutionContext,
  CoreToolResult,
  JsonObject,
  SerializableResult
} from "@deepseek/platform-contracts";
import { boundedText, defineToolManifest, failure, objectSchema, replay, success } from "../../shared/tool-kit.js";
import { coreToolIds } from "../../shared/ids.js";
import { synthesizeAgentWorkOrder, validateAgentWorkOrder } from "../../shared/agent-work-order.js";
import type { CoreCodingToolsDependencies } from "../../shared/workspace.js";
import { requireDeps } from "../../shared/workspace.js";

export interface AgentContinueToolDeps extends CoreCodingToolsDependencies {
  readonly agentSpawner?: AgentSpawner;
}

export function defineAgentContinueTool(deps: AgentContinueToolDeps | undefined) {
  return defineToolManifest(
    "agent.continue",
    coreToolIds.agentContinue,
    "Agent Continue",
    "process",
    ["agent:continue"],
    objectSchema(["workerInstanceId", "prompt"], {
      workerInstanceId: { type: "string" },
      prompt: { type: "string" },
      workOrder: { type: "object" },
      workOrderId: { type: "string" },
      parentSessionId: { type: "string" },
      parentAgentId: { type: "string" },
      parentAgentInstanceId: { type: "string" },
      toolProjection: { type: "string" },
      toolScope: { type: "object" },
      contextScope: { type: "object" },
      timeoutMs: { type: "number" },
      maxIterations: { type: "number" },
      contextOverlapScore: { type: "number" },
      evidenceIds: { type: "array" },
      reasonCode: { type: "string" },
      reason: { type: "string" }
    }),
    objectSchema(["evidence"], { evidence: { type: "object" } }),
    (input, context) => requireDeps(deps).then((ready) => agentContinueTool(input, context, ready as AgentContinueToolDeps))
  );
}

async function agentContinueTool(input: JsonObject, context: CapabilityExecutionContext, deps: AgentContinueToolDeps): Promise<SerializableResult<CoreToolResult>> {
  const parsed = input as AgentContinueRequest;
  if (!deps.agentSpawner?.continue) {
    return failure("agent.continue", "AGENT_CONTINUE_UNAVAILABLE", "No AgentSpawner continue handler registered in runtime dependencies.", []);
  }
  try {
    const workOrder = parsed.workOrder ?? synthesizeAgentWorkOrder({
      prompt: parsed.prompt,
      agentMode: "worker",
      ...(parsed.toolProjection ? { toolProjection: parsed.toolProjection } : {}),
      ...(parsed.toolScope ? { toolScope: parsed.toolScope } : {}),
      ...(parsed.contextScope ? { contextScope: parsed.contextScope } : {}),
      ...(parsed.parentSessionId ? { parentSessionId: parsed.parentSessionId } : {}),
      ...(parsed.parentAgentId ? { parentAgentId: parsed.parentAgentId } : {}),
      ...(parsed.workOrderId ? { workOrderId: parsed.workOrderId } : {}),
      ...(parsed.reason ? { reason: parsed.reason } : {})
    }, context);
    const diagnostics = validateAgentWorkOrder(workOrder);
    if (diagnostics.length > 0) {
      return failure("agent.continue", diagnostics[0]?.code ?? "AGENT_WORK_ORDER_INVALID", diagnostics[0]?.message ?? "Agent work order is invalid.", [], { diagnostics });
    }
    const result = await deps.agentSpawner.continue({
      ...parsed,
      workOrder,
      workOrderId: parsed.workOrderId ?? workOrder.workOrderId,
      ...(parsed.parentSessionId ?? context.envelope.sessionId ? { parentSessionId: parsed.parentSessionId ?? context.envelope.sessionId } : {}),
      toolProjection: parsed.toolProjection ?? "read-only"
    });
    return success("agent.continue", [String(result.childSessionId)], {
      preview: boundedText(result.assistantText, 16_000),
      metadata: {
        childSessionId: result.childSessionId,
        workerAgentId: result.workerAgentId,
        workerInstanceId: result.workerInstanceId,
        continuationOf: result.continuationOf,
        workOrderId: result.workOrderId,
        agentMode: result.agentMode,
        terminalStatus: result.terminalStatus,
        iterations: result.iterations,
        toolCalls: result.toolCalls,
        usage: result.usage,
        resultProvenance: result.resultProvenance,
        workerResult: result.workerResult as unknown as JsonObject,
        verifierStatus: result.verifierStatus,
        diagnostics: result.diagnostics as unknown as JsonObject
      },
      replay: replay(context),
      status: result.terminalStatus === "completed" ? "completed" : "failed"
    });
  } catch (error) {
    return failure("agent.continue", "AGENT_CONTINUE_FAILED", error instanceof Error ? error.message : "Agent continue failed.", []);
  }
}
