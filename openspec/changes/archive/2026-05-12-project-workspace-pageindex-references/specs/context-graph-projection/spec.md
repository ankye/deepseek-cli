...
ContextGraph projection SHALL materialize PageIndex-shaped `turn` references into bounded `summary` context nodes, SHALL preserve explicit recall scope provenance, and SHALL leave other unsupported references as evidence-only.

ContextGraph projection 必须将 PageIndex 形态的 `turn` references 物化为有界 `summary` context nodes，必须保留明确的 recall scope provenance，并将其他 unsupported references 保持为 evidence-only。
