...
- 新增 `core.skill.list` tool(read-only):调 `deps.skills.listSummaries()`(`SkillSystem` 已暴露的 list 方法),返回按 `manifest.name` 字母序排序的 `SkillSummary[]`,让 agent 在不触发激活的前提下枚举。
...
- Spec delta:`skill-system` 新增「`listSummaries` 按 manifest.name 排序」requirement + 一条「explicit activation is agent-visible via tool」requirement。
- Spec delta:`core-coding-tools` 新增 `skill.list` / `skill.activate` 两条 requirement。

## Impact

- 受影响规范:`skill-system`(`listSummaries` 排序保证 + agent-visible activation)、`core-coding-tools`(新 2 个 tool)。
- 受影响代码:
  - `@deepseek/platform-contracts`:`RuntimeEventKind` 加 `skill.activated`;`CoreCodingToolName` 加 `skill.list` + `skill.activate`。`SkillSystem.listSummaries()` 接口已存在,仅补排序契约。
  - `@deepseek/skill-system`:`InMemorySkillSystem.listSummaries()` 补按 `manifest.name` 升序排序。
...
- 测试:`tests/contracts/skill-tools.test.ts`(4 case:listSummaries 空列表、activate 成功返回 segments、activate 未知 skill 返回 not-found、failure 路径)。
