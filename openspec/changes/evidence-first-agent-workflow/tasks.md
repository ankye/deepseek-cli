...
- [ ] 3.5 Add tests for stale, missing, duplicate, secret-like, out-of-scope, and over-budget evidence candidate exclusions.
...
- [ ] 4.4 Add model feedback path for unsupported strict claims so the agent can revise once before failing closed under retry policy.
- [ ] 4.5 Add runtime integration tests for evidence-first fact-sensitive run, speculative run, prompt-boundary preservation, and unsupported-claim retry/failure.

## 5. Claim Grounding And Unsupported Claim Detection

- [ ] 5.1 Implement deterministic extraction for high-value strict claims: commands, package names, executable names, feature bullets, install instructions, release state, architecture claims, and evaluation conclusions.
- [ ] 5.2 Implement claim certainty classification as `verified`, `inferred`, `assumption`, or `unsupported`.
...
- [ ] 7.4 Add competitive/evaluation report handling that separates verified evidence, inference, assumptions, and unsupported claims.

## 8. Regression And Replay

- [x] 8.1 Add contract tests for evidence manifests and unsupported claim diagnostics.
- [ ] 8.2 Add runtime golden replay tests for evidence event ordering and prompt-boundary preservation.
...
- [ ] 9.1 Gate evidence-first behavior behind config/request activation for initial implementation, then enable by default for fact-sensitive `deepseek run`.
- [ ] 9.2 Enable evidence-first behavior for chat turns after scripted chat evidence tests pass.
...
- [ ] 9.6 Refresh acceptance evidence if diagnostics or release-readiness output changes.
