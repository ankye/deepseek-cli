...
- **THEN** the tool calls `deps.skills.listSkills()`, returns the count and per-skill `{id, name, version, trust, loadingState}` summaries, and does not trigger any activation side effect
- **中文** 当 `core.skill.list` 被调用时,tool 必须调用 `deps.skills.listSkills()`,返回数量与每个 skill 的 `{id, name, version, trust, loadingState}` summary,且不触发任何激活副作用。
