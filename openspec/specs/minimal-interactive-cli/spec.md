# minimal-interactive-cli Specification

## Purpose

Defines the R1 minimal terminal interactive shell for DeepSeek CLI. The shell is a host adapter over runtime/protocol events and must not become an independent execution engine.

定义 DeepSeek CLI 的 R1 最小终端交互式 shell。该 shell 是 runtime/protocol events 之上的 host adapter，不得成为独立执行引擎。
