# context-pipeline-prefix-cache Specification

## Purpose
Define the immutable layered context pipeline, prefix hash manifest, provider-neutral cache evidence, volatile tail isolation, and bounded CLI/TUI telemetry that let DeepSeek preserve stable model prefixes across turns.

定义不可变分层上下文管道、prefix hash manifest、provider-neutral 缓存证据、易变尾部隔离，以及有界 CLI/TUI telemetry，使 DeepSeek 能跨 turn 保持稳定模型前缀。
